...
export class RuntimeV2Repository {
  ...
  replace_vectors(items) {
    ...
    return this.syncRows({ items: items.filter((item) => clean(item.memoryId) && (list(item.vector).length || list(item.embedding).length)), table: "v2_memory_vectors", idColumn: "memory_id", hashColumn: "vector_hash", itemId: (item) => clean(item.memoryId), itemHash: (item) => sha256(vectorBuffer(list(item.vector).length ? item.vector : item.embedding)), upsert: (item, _i, id, hash) => { const field = list(item.vector).length ? "vector" : "embedding"; const values = item[field]; const metadata = { ...item }; delete metadata.vector; delete metadata.embedding; upsert.run(id, clean(item.charId) || null, clean(item.model), Number(item.dimensions || values.length), vectorBuffer(values), hash, field, Object.hasOwn(item, "dimensions") ? 1 : 0, iso(item.updatedAt), JSON.stringify(metadata)); }, tombstone: (id, at) => this.db.prepare("UPDATE v2_memory_vectors SET deleted_at=? WHERE memory_id=?").run(at, id) });
  }
